...
sin_posiciones_cortas <- function(X_past, X_reales, modelos, k, gamma) {

  mod_osa <- oneStepAhead_mod(X_past, X_reales, modelos, k)
  mu_matrix  <- as.matrix(mod_osa$mean)
  Sigma_list <- construccion_sigma_t(X_past, X_reales, modelos, k)
  ...
  for (i in 1:k) {
    ...
    alphas[[i]] <- sol$solution
  }
  
  alphas_matrix <- do.call(rbind, alphas)
  rownames(alphas_matrix) <- paste0("t+", 1:k)
  colnames(alphas_matrix) <- colnames(mu_matrix)

  alphas_matrix <- round(alphas_matrix, 8)
  alphas_matrix[alphas_matrix < 0] <- 0
