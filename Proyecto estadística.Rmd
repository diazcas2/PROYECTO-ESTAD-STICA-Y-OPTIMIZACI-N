...
getAlphaLog <- function(mu, Sigma, gamma){
  # INPUT:
  # mu: vector en R^5 con los rendimientos esperados
  # Sigma: matriz de covarianza 5x5
  # gamma: aversión al riesgo
  
  N <- length(mu)
  penalty_weight <- 1e4   # penalización fuerte para asegurar suma=1
  
  # Función objetivo penalizada
  f_obj <- function(alpha){
    alpha <- matrix(alpha, ncol = 1)
    muTalpha <- as.numeric(t(alpha) %*% mu)
    alphaTSigmaAlpha <- as.numeric(t(alpha) %*% Sigma %*% alpha)
    
    # Utilidad log aproximada (Taylor)
    Ulog <- log(1 + muTalpha) - (gamma/2) * alphaTSigmaAlpha / ((1 + muTalpha)^2)
    
    # Penalización por violar la suma = 1
    penalty <- penalty_weight * (sum(alpha) - 1)^2
  ...
  # Optimización sin límites (permitimos cortas)
  sol <- optim(par = alpha0, fn = f_obj, method = "BFGS")
